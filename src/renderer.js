...
ipcRenderer.on('youtube', async (ev, arg) => {
  ...
  frame.addEventListener('load', function () {
    setTimeout(() => {
      if (document.getElementById('video').contentDocument.querySelector('.ytp-error-content-wrap-reason')) { ipcRenderer.send('openLink', 'https://youtube.com/watch?v=' + arg) }
    }, 2000)
  })
})
